...
procedure draw_logo(time_delay : integer);  {Vẽ Logo Game, time_delay là thời gian nghỉ giữa các dòng}
    ...
    begin
        assign(logo,'E:\dung\Pascal\tai_xiu\logo.txt'); reset(logo);
...
procedure draw_menu;    {Vẽ Menu Game}
    ...
    begin
        assign(menu,'E:\dung\Pascal\tai_xiu\menu.txt'); reset(menu);
...
procedure player_config;    {Nhập dữ liệu đầu vào}
    var i, num_player, y, du_doan_so : integer;
        name_player: array [1..100] of string;
        player_predicted : array [1..100] of string;
        nguoi_choi, du_doan : text;

    begin
        assign(du_doan, 'E:\dung\Pascal\tai_xiu\du_doan.txt'); rewrite(du_doan);
        assign(nguoi_choi, 'E:\dung\Pascal\tai_xiu\nguoi_choi.txt'); rewrite(nguoi_choi);
...
procedure game_logic;    {Xử lý, đưa ra kết quả}                    
    var result  : string;
        i, num_player, count : integer;
        ket_qua, du_doan, nguoi_choi : text;
        ds_nguoi_choi : array[1..100] of string;
        ds_du_doan : array[1..100] of string; 

    begin
        assign(ket_qua, 'E:\dung\Pascal\tai_xiu\ket_qua.txt'); reset(ket_qua);
        assign(du_doan, 'E:\dung\Pascal\tai_xiu\du_doan.txt'); reset(du_doan);
...
procedure game;    {Random kết quả}
    var i, out : integer;
        ket_qua, game_mode: text;
        mode : integer;
        
    begin
        {Đọc dữ liệu cần thiết}
        assign(ket_qua,'E:\dung\Pascal\tai_xiu\ket_qua.txt'); rewrite(ket_qua);
        assign(game_mode, 'E:\dung\Pascal\tai_xiu\game_mode.txt'); reset(game_mode);
...
var key_inp, a  : string;
    game_mode, end_menu : text;

begin   
    assign(game_mode, 'E:\dung\Pascal\tai_xiu\game_mode.txt'); rewrite(game_mode);
    ...
    repeat
        begin
            assign(end_menu,'E:\dung\Pascal\tai_xiu\end_menu.txt'); reset(end_menu);
